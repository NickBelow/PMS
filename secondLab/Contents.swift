...
for student in studentsArray{
	let pair = student.components(separatedBy: " - ")
	students.updateValue(pair[1], forKey: pair[0] )
	groups.insert(pair[1])
}
for each in groups{
	var buffer: [String] = []
	for student in students{
		if student.value==each{
			buffer.append(student.key)
		}
	}
	studentsGroups.updateValue(buffer, forKey: each)
...
for group in studentsGroups{
	...
	for student in Array(group.value){
		var studentMarks:[Int] = []
		for _ in 1...9{
			studentMarks.append(randomValue(maxValue:16))
		}
		array.updateValue(studentMarks, forKey: student)
	}
	studentPoints.updateValue(array, forKey: group.key)
}
...
for group in studentPoints{
	...
	for student in group.value{
		var mark:Int = 0
		for i in student.value{
			mark+=i
		}
		array.updateValue(mark, forKey: student.key)
	}
	sumPoints.updateValue(array, forKey: group.key)
}
...
for group in sumPoints{
	var averageMark : Float = 0
	var number : Int = 0
	for student in group.value{
		averageMark += Float(student.value)
		number += 1
		}
	groupAvg.updateValue(averageMark/Float(number), forKey: group.key)
}
...
for group in sumPoints{
	...
	for student in group.value{
		if(student.value >= 60){
			array.append(student.key)
		}
	}
	passedPerGroup.updateValue(array, forKey: group.key)
}
