    ...
    const apiSearchFunction = async (text) => {
        ...
        if (filteredText.length < 3) {
            ...
            if ((await Network.getNetworkStateAsync()).isConnected) {
                ...
                zminna = response.Search;
            } else {
                zminna = cachedContent;
            }
            
            if (zminna !== undefined) {
                setSelectedData(getUniqueListBy(zminna, 'imdbID'))
                // handleAddFilm(getUniqueListBy(zminna, 'imdbID'))
            }
        }
    };
